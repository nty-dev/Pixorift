...
AIgraph = os.path.join(os.path.dirname(__file__), 'TrainedAI/graph.pb')
imgvar = os.path.join(os.path.dirname(__file__), 'TrainedAI/var.txt')

class PixoAI():
    def __init__(self, AI=AIgraph, imgvar=imgvar):
    ...
    def checksum(self, a=AIgraph, b=imgvar):
        if not os.path.exists(a):
            print('PixoAI Error: A fatal error has occured. Machine Learning data does not exist.')
            return [False, 'PixoAI Error: A fatal error has occured. Machine Learning data does not exist.']
        if not os.path.exists(b):
            print('PixoAI Error: AI variable file does not exist.')
            return [False, 'PixoAI Error: AI variable file does not exist.']
        return [True]
